...
{
  return ( Index >= 0 ) && ( Index <= ( int ) Path.Length() ) && ( Path[ Index ] == PathDelim );
}
...
wxString FileSearch ( wxString const& name, wxString const& DirList )
{
  ...
  do
  {
    ...
    if (  ( Result.Length() > 0 ) && ( Result[ Result.Length() ] != DirectorySeparator ) )
...
bool AnsiSameText ( wxString const& S1, wxString const& S2 ) /*?*//*$ifdef SYSUTILSINLINE}{$endif*/
{
  return ( AnsiCompareText ( S1, S2 ) == 0 );
}

bool AnsiSameStr ( wxString const& S1, wxString const& S2 ) /*?*//*$ifdef SYSUTILSINLINE}{$endif*/
{
...
wxString FloatToStrF ( double Value,  TFloatFormat Format,  int Precision, int Digits )
{
  ...
  switch ( Format )
  {
    ...
    case ffCurrency:
      FormatStr = wxString::Format ( wxT("%s%%d.%df"), CurrencyString.c_str(), Precision, Digits );
      break;
  }
...
Currency FloattoCurr ( double const& Value )
{
  ...
  if (  !TryFloatToCurr ( Value , Result ) )
    throw EConvertError::CreateFmt ( SInvalidCurrency , FloatToStr ( Value ).c_str() );
  return Result;
}
...
wxDateTime DoEncodeDate ( unsigned short Year, unsigned short Month, unsigned short Day )
{
  wxDateTime Result;
...
wxDateTime DoEncodeTime ( unsigned short Hour, unsigned short Minute, unsigned short Second, unsigned short MilliSecond )
{
  wxDateTime Result;
...
wxDateTime EncodeDate ( unsigned short Year, unsigned short Month, unsigned short Day )
{
  wxDateTime Result;
  if (  !TryEncodeDate ( Year, Month, Day, Result ) )
    throw EConvertError::CreateFmt ( wxT("%d-%d-%d is not a valid date specification") , Year, Month, Day );
  return Result;
}
...
wxDateTime EncodeTime ( unsigned short Hour, unsigned short Minute, unsigned short Second, unsigned short MilliSecond )
{
  wxDateTime Result;
  if (  !TryEncodeTime ( Hour, Minute, Second, MilliSecond, Result ) )
    throw EConvertError::CreateFmt ( wxT("%d:%d:%d.%d is not a valid time specification") , Hour, Minute, Second, MilliSecond );
  return Result;
}
...
{
  wxDateTime Result = ComposeDateTime ( DoEncodeDate ( SystemTime.Year , SystemTime.Month , SystemTime.Day ) , DoEncodeTime ( SystemTime.Hour , SystemTime.Minute , SystemTime.Second , SystemTime.MilliSecond ) );
  return Result;
}
...
{
  wxDateTime Result = DateTime.Add ( wxDateSpan ( 0, 0, 0, 1 ) );
  return Result;
}
...
{
  wxString Result = FormatDateTime ( wxT("ddddd") , Date );
  return Result;
}

wxString TimeToStr ( wxDateTime Time )
{
  return FormatDateTime ( wxT("tt") , Time );
}

wxString DateTimeToStr ( wxDateTime DateTime )
{
  return FormatDateTime ( wxT("c") , DateTime );
}
...
{
  wxDateTime Result ( (time_t)Filedate );
  return Result;
}
...
{
  dati.Set ( NewTime.GetHour(), NewTime.GetMinute(), NewTime.GetSecond(), NewTime.GetMillisecond() );
}
...
{
  if ( Global )
    return IncludeTrailingPathDelimiter ( wxStandardPaths::Get().GetConfigDir() );
  else
    return IncludeTrailingPathDelimiter ( wxStandardPaths::Get().GetUserConfigDir() );
}
...
unsigned long GetTempFileName ( const wxString &Dir, const wxString &Prefix,  const unsigned long uUnique,  wxString &TempFileName )
{
...
bool ForceDirectories ( wxString const& Dir )
{
  ...
  PartialFullDir = PartialFullDir.Format ( wxT("%s%s%c%s%c"), fn.GetVolume().c_str(), fn.GetVolumeSeparator().c_str(), fn.GetPathSeparator(), aryString.Item ( 0 ).c_str(), fn.GetPathSeparator() );
  ...
  {
    PartialFullDir = PartialFullDir.Format ( wxT("%s%s%c"), PartialFullDir.c_str(), aryString.Item ( i ).c_str(), fn.GetPathSeparator() );
    if ( wxDir::Exists ( PartialFullDir ) == false )
    {
      fn.Mkdir ( PartialFullDir );
    }
...
{
  return wxString::Format(wxT("%d,%d,%d,%d"),Value.GetYears(),Value.GetMonths(),Value.GetWeeks(),Value.GetDays());
}
...
{
  return wxString::Format(wxT("%d,%d,%d,%d"),Value.GetX(),Value.GetY(),Value.GetHeight(),Value.GetWidth());
}
...
wxDateSpan StrToDateSpan( const wxString& Value)
{
  ...
  int weeks = 0;
  int days = 0;
  ::wxSscanf(Value.c_str(),wxT("%d,%d,%d,%d"),&years , &months , &weeks , &days);
...
{
  wxRect Result;
  ::wxSscanf(Value.c_str(),wxT("%d,%d,%d,%d"),&Result.x,&Result.y,&Result.height,&Result.width);
  return Result;
}
