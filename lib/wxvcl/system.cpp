...
void UniqueString ( wxString & s )
{
  ...
  s = ExtractFileNameOnly(wxFileName::CreateTempFileName(wxString::Format(wxT("%d"),rand())));
}
...
wxString Concat ( wxString const& S1, wxString const& S2, wxString const& S3, wxString const& Sn )
{
  return ( S1 + S2 + S3 + Sn );
}
