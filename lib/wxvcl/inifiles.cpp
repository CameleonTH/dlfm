...
bool TIniFile::IsSection(const wxString &Line)
{
    ...
    {
        S = Trim( Line );
        if(  ( S[ 0 ] == IniBrackets[ 0 ] ) && ( S[ (unsigned int) Length( S ) -1 ] == IniBrackets[ 1 ] ) )
            result = true;
    }
    return result;
}
...
{
    int result;
    result = FFileBuffer->IndexOf( IniBrackets[ 0 ] + Section + IniBrackets[ 1 ],true );
    return result;
}
...
wxString TIniFile::ReadString(const wxString &Section, const wxString &Ident,  const wxString &Default)
{   wxString result;
    int I;
    wxString V;
...
long TIniFile::ReadInteger(const wxString &Section, const wxString &Ident,  long Default)
{
    long result;
    wxString IntStr;
    IntStr = ReadString( Section , Ident , wxT("") );
    // convert a Hex-Value
    if(  ( Length( IntStr ) > 2 ) && ( IntStr.GetChar(1) == '0' ) && ( ( IntStr.GetChar(2) == 'X' ) || ( IntStr.GetChar(2) == 'x' ) ) )
...
bool TIniFile::ReadBool(const wxString &Section,const wxString &Ident,  bool Default)
{
    bool result;
...
float TIniFile::ReadFloat(const wxString &Section, const wxString &Ident,  float Default)
{
      return StrToFloat(ReadString(Section, Ident,FloatToStr(Default)));
}
double TIniFile::ReadDouble(const wxString &Section, const wxString &Ident,  double Default)
{
 return StrToDouble(ReadString(Section, Ident,DoubleToStr(Default)));       
}
unsigned long TIniFile::ReadULong(const wxString &Section, const wxString &Ident,  unsigned long Default)
{
 return StrToULong(ReadString(Section, Ident,ULongToStr(Default)));         
}

long TIniFile::ReadLong(const wxString &Section, const wxString &Ident, long Default)
{
 return StrToLong(ReadString(Section, Ident,LongToStr(Default)));
}

wxDateTime TIniFile::ReadDateTime(const wxString &Section, const wxString &Ident, wxDateTime Default)
{
 return StrToDateTime(ReadString(Section, Ident,DateTimeToStr(Default)));
}
 
wxDateTime TIniFile::ReadULongDateTime(const wxString &Section, const wxString &Ident, wxDateTime Default)
{	
...
wxDateTime TIniFile::ReadDate(const wxString &Section, const wxString &Ident, wxDateTime Default)
{
 return StrToDateTime(ReadString(Section, Ident,DateToStr(Default)));
}
wxDateTime TIniFile::ReadTime(const wxString &Section, const wxString &Ident, wxDateTime Default)
{
 return StrToDateTime(ReadString(Section, Ident,TimeToStr(Default)));
}

wxColour TIniFile::ReadColor( const wxString & Section, const wxString & Ident,  wxColour Default)
{
 return StrToColor(ReadString(Section, Ident,ColorToStr(Default)));   
}
wxFont TIniFile::ReadFont( const wxString & Section, const wxString & Ident,  wxFont Default)
{
 return StrToFont(ReadString(Section, Ident,FontToStr(Default)));       
}
wxPoint TIniFile::ReadPoint( const wxString & Section, const wxString & Ident,  wxPoint Default)
{
     return StrToPoint(ReadString(Section, Ident,PointToStr(Default)));   
}
wxRealPoint TIniFile::ReadRealPoint( const wxString & Section, const wxString & Ident,  wxRealPoint Default)
{
     return StrToRealPoint(ReadString(Section, Ident,RealPointToStr(Default)));   
}
wxRect TIniFile::ReadRect( const wxString & Section, const wxString & Ident,  wxRect Default)
{
    return StrToRect(ReadString(Section, Ident,RectToStr(Default)));   
}

wxSize TIniFile::ReadSize( const wxString & Section, const wxString & Ident,  wxSize Default)
{
...
void TIniFile::ReadSectionNamesValues(const wxString &Section,  TStringList &NameStrings,  TStringList &ValueStrings)
{
    wxString N; wxString V;
    int I;
...
void TIniFile::WriteString( const wxString &Section, const wxString &Ident,  const wxString &Value)
{
    int I;
    ...
    if(  I != -1 )
    {
        ++I;
        while(  ( I < FFileBuffer->Count() ) && ! IsSection( FFileBuffer->Item(I) ) &&
                ( GetName( FFileBuffer->Item(I) ) != Ident ) ) ++I;
        // End of File or Ident doesn't exists in the Section
        if(  ( I >= FFileBuffer->Count() ) || IsSection( FFileBuffer->Item(I) ) )
        {
            if(  Ident != EmptyStr ) FFileBuffer->Insert( I , Ident + IniSeparator + Value );
        }
        // Ident does exists in the section
        else if(  Ident != EmptyStr ) FFileBuffer->Item(I) = Ident + IniSeparator + Value;
    }
    ...
    else
    {
        ...
        if(  Ident != EmptyStr ) FFileBuffer->Add( Ident + IniSeparator + Value );
    }
    SaveToFile();
}
...
void TIniFile::WriteInteger( const wxString &Section, const wxString &Ident,  long Value)
{
...
void TIniFile::WriteBool(const wxString &Section, const wxString &Ident,  bool Value)
{
    wxString StrValue;
    if(  ( Value == true ) )
        StrValue = wxT("1");
...
void TIniFile::WriteFloat(const wxString &Section, const wxString &Ident,  float Value)
{
 WriteString( Section , Ident , FloatToStr( Value ) );     
}

void TIniFile::WriteDouble(const wxString &Section, const wxString &Ident,  double Value)
{
 WriteString( Section , Ident , DoubleToStr( Value ) );          
}

void TIniFile::WriteULong(const wxString &Section,const wxString &Ident,  unsigned long Value)
{
 WriteString( Section , Ident , ULongToStr( Value ) );               
}

void TIniFile::WriteLong(const wxString &Section,const wxString &Ident,  long Value)
{
 WriteString( Section , Ident , LongToStr( Value ) );
}

void TIniFile::WriteDateTime(const wxString &Section,const wxString &Ident,  wxDateTime Value)
{
 WriteString( Section , Ident , DateTimeToStr( Value ) );
}
void TIniFile::WriteULongDateTime(const wxString &Section,const wxString &Ident,  wxDateTime Value)
{
 WriteULong( Section , Ident , Value.GetTicks() );
}
void TIniFile::WriteDate(const wxString &Section,const wxString &Ident,  wxDateTime Value)
{
 WriteString( Section , Ident , DateToStr( Value ) );
}
void TIniFile::WriteTime(const wxString &Section,const wxString &Ident,  wxDateTime Value)
{
 WriteString( Section , Ident , TimeToStr( Value ) );
}

void TIniFile::WriteDateSpan( const wxString & Section, const wxString & Ident,  wxDateSpan Value)
{
  WriteString( Section , Ident , DateSpanToStr( Value ) );
}
void TIniFile::WriteColor( const wxString & Section, const wxString & Ident,  wxColour Value)
{
  WriteString( Section , Ident , ColorToStr( Value ) );
}
void TIniFile::WriteFont( const wxString & Section, const wxString & Ident,  wxFont Value)
{
  WriteString( Section , Ident , FontToStr( Value ) );
}
void TIniFile::WritePoint( const wxString & Section, const wxString & Ident,  wxPoint Value)
{
  WriteString( Section , Ident , PointToStr( Value ) );
}
void TIniFile::WriteRealPoint( const wxString & Section, const wxString & Ident,  wxRealPoint Value)
{
  WriteString( Section , Ident , RealPointToStr( Value ) );
}
void TIniFile::WriteRect( const wxString & Section, const wxString & Ident,  wxRect Value)
{
  WriteString( Section , Ident , RectToStr( Value ) );
}
void TIniFile::WriteSize( const wxString & Section, const wxString & Ident,  wxSize Value)
{
  WriteString( Section , Ident , SizeToStr( Value ) );
}

void TIniFile::WriteStringList( const wxString & Section, const wxString & Ident,  wxArrayString Value)
{
  //fixme:
}
...
void TIniFile::EraseSection(const wxString &Section)
{
    ...
    if(  I != -1 )
    {
        ...
        while(  ( I < FFileBuffer->Count() ) && ! IsSection( FFileBuffer->Item(I) ) )
            FFileBuffer->Delete( I );
        if(  I > 0 ) FFileBuffer->Insert( I , EmptyStr );
        SaveToFile();
    }
}
