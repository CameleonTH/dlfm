...
bool wxSelectListItem(wxListCtrl *lstvw, int Row)
{
	...
	{
		Result = true;
		lstvw->SetItemState(Row,wxLIST_STATE_FOCUSED | wxLIST_STATE_SELECTED, wxLIST_STATE_FOCUSED | wxLIST_STATE_SELECTED);
	}
	return Result;
}
...
int GetTreeItemSelectedCount(wxTreeCtrl *tvCtrl,int SelImageIdx, const wxTreeItemId& idParent, wxTreeItemIdValue cookie)
{
	int Result = 0;
	wxTreeItemId id;
	if ( !cookie )
...
void SelectAllTreeItems(const wxString &searchStr,wxTreeCtrl *tvCtrl,int SelImageIdx, const wxTreeItemId& idParent, wxTreeItemIdValue cookie)
{
	wxTreeItemId id;
	if ( !cookie )
		id = tvCtrl->GetFirstChild(idParent, cookie);
...
void InverseSelectedTreeItems(wxTreeCtrl *tvCtrl,int SelImageIdx, int UnSelImageIdx,const wxTreeItemId& idParent, wxTreeItemIdValue cookie)
{
	wxTreeItemId id;
	if ( !cookie )
		id = tvCtrl->GetFirstChild(idParent, cookie);
	...
	if (tvCtrl->GetItemImage(id) == SelImageIdx)
	{
		...
		if (tvCtrl->GetItemImage(id) == UnSelImageIdx)
		{
			tvCtrl->SetItemImage(id,SelImageIdx);
		}
		...
		InverseSelectedTreeItems(tvCtrl,SelImageIdx,UnSelImageIdx,idParent, cookie);
